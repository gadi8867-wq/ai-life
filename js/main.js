...
const dayColor = new THREE.Color(0x79a9ad);
const nightColor = new THREE.Color(0x08131e);
...
function addTree(x, z, scale = 1, variant = 0) {
  ...
  const crown = new THREE.Mesh(new THREE.DodecahedronGeometry(1.7, 1), variant % 2 ? mat.leafB : mat.leafA);
  crown.position.y = 3.15; crown.scale.y = 1.18; crown.castShadow = true; crown.receiveShadow = true; g.add(crown);
  if (variant % 3 === 0) { const crown2 = crown.clone(); crown2.scale.setScalar(0.68); crown2.position.set(.75,4.15,-.25); g.add(crown2); }
  terrain.add(g);
}
function addRock(x, z, scale = 1) {
  const r = new THREE.Mesh(new THREE.DodecahedronGeometry(.75, 0), mat.rock);
  r.position.set(x, .45 * scale, z); r.scale.set(scale*rand(.8,1.35),scale*rand(.65,1),scale*rand(.75,1.25)); r.rotation.set(rand(-.3,.3),rand(0,Math.PI),rand(-.2,.2)); r.castShadow=true; r.receiveShadow=true; terrain.add(r);
}
function addGrass(x,z,scale=1){
  const g=new THREE.Group();g.position.set(x,0,z);g.scale.setScalar(scale);
  for(let i=0;i<4;i++){const blade=new THREE.Mesh(new THREE.ConeGeometry(.055,rand(.35,.65),4),mat.grass);blade.position.set(rand(-.22,.22),blade.geometry.parameters.height/2,rand(-.22,.22));blade.rotation.z=rand(-.25,.25);g.add(blade)} terrain.add(g);
}
...
for(let i=0;i<5;i++){const log=new THREE.Mesh(new THREE.CylinderGeometry(.16,.2,2,7),mat.trunk);log.position.y=.22;log.rotation.z=Math.PI/2;log.rotation.y=i*.65;log.rotation.x=.18;log.castShadow=true;fire.add(log)}
...
class AgentBrain{
  ...
  observe(world){const a=this.agent,o=agents.find(x=>x!==a),distance=o?dist2(a.root.position,o.root.position):999;return{self:{x:a.root.position.x,z:a.root.position.z},daylight:world.daylight,distanceToOther:distance,state:a.state,needs:{...a.needs},recentMemory:a.memory.slice(-4)}}
  decide(world,now){const a=this.agent,o=agents.find(x=>x!==a),d=o?dist2(a.root.position,o.root.position):999;const choices=[{w:a.needs.thirst*1.9,action:'water'},{w:a.needs.energy*1.4,action:'rest'},{w:a.needs.curiosity*daylightFactor(world.daylight)*1.5,action:'explore'},{w:(d<10?.65:.06)*a.needs.social,action:'social'},{w:.25+Math.random()*.2,action:'wander'}];choices.sort((x,y)=>y.w-x.w);const top=choices[0];this.lastThought=top.action;a.state=top.action==='water'?'seeking water':top.action==='rest'?'resting':top.action==='social'?'observing another mind':top.action==='explore'?'exploring':'wandering';a.stateUntil=now+rand(5000,11000);a.stateStarted=now;if(top.action==='water')a.target.set(rand(0,8),0,rand(-40,40));else if(top.action==='rest')a.target.copy(a.root.position);else if(top.action==='social'&&o)a.target.copy(o.root.position);else a.target.set(rand(-45,45),0,rand(-45,45));a.metrics.autonomy=clamp(a.metrics.autonomy+.003);a.metrics.decision=clamp(a.metrics.decision+(top.w>.55?.002:-.0003))}
  learn(result){const a=this.agent;a.memory.push({time:Date.now(),state:a.state,result});if(a.memory.length>40)a.memory.shift();a.metrics.learning=clamp(a.metrics.learning+.006);a.metrics.exploration=clamp(a.metrics.exploration+.0015)}
}
...
let cameraMode='auto';let selectedAgent=null;let worldTime=saved?.worldTime??7*3600;let lastDecision=0;let eventLog=saved?.events||[];
...
function save(){const payload={experimentStart,worldTime,running,events:eventLog,agents:agents.map(a=>({name:a.name,x:a.root.position.x,z:a.root.position.z,metrics:a.metrics,memory:a.memory.slice(-20)}))};try{localStorage.setItem(SAVE_KEY,JSON.stringify(payload))}catch{}}
...
function frameTarget(){if(cameraMode==='follow'&&selectedAgent)return selectedAgent.root.position.clone().add(new THREE.Vector3(0,1,0));if(cameraMode==='auto'){const a=agents[0],b=agents[1],mid=a.root.position.clone().add(b.root.position).multiplyScalar(.5);return mid}return new THREE.Vector3(0,0,0)}
function updateCamera(dt){const target=frameTarget();if(cameraMode==='auto'){const spread=dist2(agents[0].root.position,agents[1].root.position);const desired=clamp(38+spread*.55,38,58);radius=lerp(radius,desired,dt*.5);yaw+=dt*.035}if(cameraMode==='follow')radius=lerp(radius,20,dt*1.4);const pos=new THREE.Vector3(Math.sin(yaw)*Math.cos(pitch)*radius,Math.sin(pitch)*radius,Math.cos(yaw)*Math.cos(pitch)*radius).add(target);camera.position.lerp(pos,1-Math.pow(.001,dt));camera.lookAt(target)}

/* ---------- Environment ---------- */
function updateEnvironment(dt){if(running)worldTime=(worldTime+dt*8)%86400;const phase=(worldTime/86400)*Math.PI*2-Math.PI/2;const daylight=clamp(Math.sin(phase)*.5+.5);const night=1-daylight;sun.intensity=lerp(.12,2.65,daylight);moon.intensity=lerp(.28,.02,daylight);hemi.intensity=lerp(.45,1.35,daylight);scene.background.copy(dayColor).lerp(nightColor,night*.86);scene.fog.color.copy(scene.background);scene.fog.density=lerp(.008,.017,night);fireLight.intensity=lerp(2.5,5.5,night);flame.scale.setScalar(lerp(.75,1.15,Math.sin(performance.now()*.009)*.15+.5));return{daylight,night}}

/* ---------- Autonomous loop ---------- */
function updateAgents(dt,now,world){
  agents.forEach(a=>{
    a.needs.energy=clamp(a.needs.energy+dt*.004);a.needs.thirst=clamp(a.needs.thirst+dt*.006);a.needs.curiosity=clamp(a.needs.curiosity+dt*.0015);
    if(now>a.stateUntil&&running){a.brain.decide(world,now);if(Math.random()<.35)addEvent(`${a.name} изменил решение: ${a.state}.`,a.name)}
    const p=a.root.position,target=a.target,d=dist2(p,target);
    if(a.state==='resting'){a.body.rotation.z=Math.sin(now*.001+a.phase)*.025;a.needs.energy=clamp(a.needs.energy-dt*.012);a.needs.thirst=clamp(a.needs.thirst-dt*.001)}
    else if(d>.7&&running){const dir=new THREE.Vector3(target.x-p.x,0,target.z-p.z).normalize();const speed=a.state==='observing another mind'?.55:.72;p.addScaledVector(dir,dt*speed);a.root.rotation.y=Math.atan2(dir.x,dir.z);a.needs.energy=clamp(a.needs.energy+dt*.0018);a.needs.thirst=clamp(a.needs.thirst+dt*.0012);a.metrics.exploration=clamp(a.metrics.exploration+dt*.00012)}
    else if(running){if(a.state==='seeking water'){a.needs.thirst=clamp(a.needs.thirst-dt*.025);a.metrics.survival=clamp(a.metrics.survival+.0007);a.brain.learn('water reached')}if(a.state==='exploring'){a.needs.curiosity=clamp(a.needs.curiosity-dt*.02);a.brain.learn('new place observed')}if(a.state==='observing another mind'){a.needs.social=clamp(a.needs.social-dt*.018);a.metrics.social=clamp(a.metrics.social+.0008);a.brain.learn('another agent observed')}a.stateUntil=now+rand(2500,6000)}
    const bob=Math.sin(now*.0024+a.phase)*.035;a.root.position.y=bob;a.core.scale.setScalar(1+Math.sin(now*.004+a.phase)*.08);a.ring.rotation.z+=dt*.35;projectLabel(a)
  });
  const d=dist2(agents[0].root.position,agents[1].root.position);if(d<7&&Math.random()<dt*.08){addEvent('OpenAI и Cloude находятся рядом — наблюдение без вмешательства.');agents[0].needs.social=clamp(agents[0].needs.social+.15);agents[1].needs.social=clamp(agents[1].needs.social+.15)}
}
function projectLabel(a){const v=a.root.position.clone();v.y=2.45;v.project(camera);const x=(v.x*.5+.5)*innerWidth,y=(-v.y*.5+.5)*innerHeight;a.label.style.transform=`translate(${x}px,${y}px) translate(-50%,-50%)`;a.label.style.opacity=(v.z>1||Math.abs(v.x)>1)?'0':'1'}

let last=performance.now(),saveTimer=0;
function tick(now){requestAnimationFrame(tick);const dt=Math.min(.05,(now-last)/1000);last=now;const world=updateEnvironment(dt);if(running)updateAgents(dt,now,world);else agents.forEach(projectLabel);updateCamera(dt);$('#clock').textContent=new Date().toLocaleTimeString('ru-RU',{hour12:false});updatePill();if(now-lastDecision>9000&&running){lastDecision=now;renderStats()}saveTimer+=dt;if(saveTimer>8){saveTimer=0;save()}renderer.render(scene,camera)}
...
addEvent('OpenAI и Cloude появились в мире независимо друг от друга.');addEvent('Среда создана. Цели агентам не назначены.');addEvent('AI-слой готов к подключению реальной модели без изменения мира.');addEvent('Наблюдение активно. Вмешательство человека: 0.');addEvent('AI Life 2.0 — визуальное ядро запущено.');
renderEvents();renderStats();updatePill();window.addEventListener('resize',resize);resize();requestAnimationFrame(tick);

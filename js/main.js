 ...
 * Continuous seasonal calendar for Moscow: week-to-week changes are interpolated.
 */
const trees=[];
...
function seasonInfo(date=new Date()){const parts=new Intl.DateTimeFormat('en-US',{timeZone:'Europe/Moscow',month:'numeric'}).formatToParts(date);const m=Number(parts.find(p=>p.type==='month')?.value||1)-1;if(m>=2&&m<=4)return{name:'Весна',emoji:'🌱',key:'spring',progress:0};if(m>=5&&m<=7)return{name:'Лето',emoji:'☀️',key:'summer',progress:0};if(m>=8&&m<=10)return{name:'Осень',emoji:'🍂',key:'autumn',progress:autumnProgress(date)};return{name:'Зима',emoji:'❄️',key:'winter',progress:1}}
...
/* ---------- Real solar time ----------
 * Fixed experiment location: Moscow, Russia.
 ...
 */
const SOLAR={lat:55.7558,lon:37.6173};
...
addEvent('OpenAI и Cloude появились в мире независимо друг от друга.');addEvent('Среда создана. Цели агентам не назначены.');addEvent('Реальное солнечное время синхронизировано с Москвой.');addEvent('Наблюдение активно. Вмешательство человека: 0.');addEvent('AI Life 2.0 — визуальное ядро запущено.');

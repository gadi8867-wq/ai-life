...
function renderSceneEvents(){const el=$('#sceneEvents');if(!el)return;el.innerHTML='';eventLog.slice(0,3).forEach(e=>{const row=document.createElement('div');row.className='scene-event';if(e.agent){const a=agents.find(x=>x.name===e.agent);row.innerHTML=`<span class="scene-event-agent" style="--agent-color:#${a?.color.toString(16).padStart(6,'0')||'b9cbc7'}">${e.agent}</span> · ${e.text}`;}else row.textContent=e.text;el.appendChild(row)})}
...
renderEvents();renderSceneEvents();renderStats();updatePill();window.addEventListener('resize',resize);resize();requestAnimationFrame(tick);

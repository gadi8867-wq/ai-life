...
function applySeason(date=new Date()){const s=seasonInfo(date),p=seasonPalettes[s.key];trees.forEach(t=>t.foliage.forEach((mesh,i)=>{if(s.key==='autumn'){const warm=t.warmBias;const hue=lerp(i%2===0?.28:.16,.055,warm);const sat=lerp(.44,.82,warm);const light=lerp(.25,.47,warm);mesh.material.color.copy(seasonColor([hue,sat,light]));}else{const base=i%2===0?p.leafA:p.leafB;const light=(t.warmBias-.5)*.035;mesh.material.color.copy(seasonColor([base[0],base[1],clamp(base[2]+light,.12,.62)]));}}));mat.ground.color.copy(seasonColor(p.ground));mat.grass.color.copy(seasonColor(p.grass));mat.trunk.color.copy(seasonColor(p.trunk));seasonLeaves.points.visible=s.key==='autumn';return s}

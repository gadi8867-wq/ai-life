...
const worldItems=[];
const weather={rain:false,rainUntil:0,wind:0};
...
function updateStartButton(){const b=$('#start');if(!b)return;b.classList.toggle('running',running);const icon=b.querySelector('.start-icon');const label=b.querySelector('.start-text');if(icon)icon.textContent=running?'Ⅱ':'▶';if(label)label.textContent=running?'ПАУЗА':'СТАРТ';b.setAttribute('aria-label',running?'Поставить эксперимент на паузу':'Запустить эксперимент')}
$('#start').addEventListener('click',()=>{running=!running;if(running){addEvent('Жизнь OpenAI и Cloude началась.');}else{addEvent('Жизнь остановлена наблюдателем.');}updateStartButton();save()});

...
function updateStartButton(){const b=$('#start');if(!b)return;b.classList.toggle('running',running);const icon=b.querySelector('.start-icon');const label=b.querySelector('.start-text');if(icon)icon.textContent=running?'Ⅱ':'▶';if(label)label.textContent=running?'СТОП':'СТАРТ';b.setAttribute('aria-label',running?'Остановить эксперимент':'Запустить эксперимент')}
